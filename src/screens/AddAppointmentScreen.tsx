    ...
    return (
        <ScrollView style={[styles.container, {}]}>
            ...
            <AppointmentInfo
                ...
                data={[
                    { label: "Package", value: customerInfo?.name ?? "Name" },
                    { label: "Note", value: customerInfo?.phone ?? "Phone" },
                ]}
            />
        </ScrollView>
    );
...
const AppointmentInfo = ({ openModal, title, date, data }: any) => {
    return (
        <TouchableOpacity onPress={() => openModal(`datetime`)}>
            ...
            {title === `Date & Time` ? (
                <View style={{ padding: 10 }}>
                    ...
                    >
                        {date ? moment(date).format("dddd LL") : "Date"}
                    </Text>
                    <View
                        style={{
                            flexDirection: "row",
                        ...
                        {data.map((d) => (
                            <View>
                                <Text
                                    style={[
                                        styles.header,
                                        { borderWidth: 1, borderColor: "lightgrey", padding: 5 },
                                    ]}
                                >
                                    {d.value ? moment(d.value).format("hh:mm a") : d.label}
                                </Text>
                            </View>
                        ))}
                    </View>
...
const DateAndTime = ({
    ...
    const { slotList } = useSelector(
        (state: any) => state.slotListReducer,
        shallowEqual
    );
    const { taskList } = useSelector(
        (state: any) => state.taskListReducer,
        shallowEqual
    );

    const withinRange = taskList.find((tD: Task) =>
        moment(date).isBetween(moment(tD.dateStart), moment(tD.dateEnd))
    );
    const initAvailableSlots = slotList.filter(
        (s: SlotInterface) =>
            !taskList.find(
                (tD: Task) =>
                    moment(date).isBetween(moment(tD.dateStart), moment(tD.dateEnd)) &&
    ...
    useEffect(() => {
        const newAvailableSlots = slotList.filter(
            (s: SlotInterface) =>
                !taskList.find(
                    (tD: Task) =>
                        moment(date).isBetween(moment(tD.dateStart), moment(tD.dateEnd)) &&
                    ...
                    >
                        <View>
                            <Ionicons
                                name={"md-checkbox"}
                                size={27}
                                color={item.selected === true ? "blue" : "grey"}
                            />
                        </View>
                        <View>
                            <Text>{item.name}</Text>
...
const Package = ({ openModal, setPackage, setNote }: PackageProps) => {
    ...
    const { serviceList } = useSelector(
        (state: any) => state.serviceListReducer,
        shallowEqual
    );
        ...
        >
            ...
            >
                ...
                <TextInput
                    ...
                    multiline={true}
                    numberOfLines={3}
                    style={{ borderWidth: 1, borderColor: "lightgrey", height: 50 }}
                />
            </View>
...
const CustomerInfo = ({
    ...
    const { customerList } = useSelector(
        (state: any) => state.customerListReducer,
        shallowEqual
    );
    ...
    const filteredCustomerList = name
        ? customerList.filter((c: CustomerInterface) =>
    ...
    return (
        ...
        >
            ...
            >
                ...
                <TextInput
                    onChangeText={(value) => setNewCustomerDetail({ name: value })}
                    style={{ borderWidth: 1, borderColor: "lightgrey", height: 44 }}
                />
            </View>
            <View
                style={{
            ...
            >
                ...
                <TextInput
                    onChangeText={(value) => setNewCustomerDetail({ phone: value })}
                    style={{ borderWidth: 1, borderColor: "lightgrey", height: 44 }}
                />
            </View>
            <Button onPress={() => openModal(false)} title="Close" />
            {newCustomer && (
                <TouchableOpacity
                    onPress={() => getCustomer(newCustomer)}
                    style={{
                        flexDirection: "row",
                        alignSelf: "stretch",
                        justifyContent: "space-between",
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <TouchableOpacity
                        onPress={() => getCustomer({ name: item.name, phone: item.phone })}
                        style={{
                            flexDirection: "row",
                            alignSelf: "stretch",
                            justifyContent: "space-between",
