...
export default function TabOneScreen({ navigation }: any) {
  ...
  days.map((d) => {
    const tasks: Array<Task> = taskList?.filter((tD: Task) =>
      moment(tD.dateStart).isSame(d, "day")
    );
    appointmentDay.push({
      title: d.format("dddd LL"),
      data:
        tasks?.length > 0
          ? tasks
          : [
    ...
    <View style={styles.container}>
      <SectionList
        SectionSeparatorComponent={() => (
          <View style={{ borderBottomWidth: 1, borderColor: "#EAEBFF" }} />
        )}
        ...
        renderSectionHeader={({ section: { title } }) => (
          <View style={{ padding: 10, backgroundColor: "#FEFAF2" }}>
...
const Item = ({ task, navigation }: any) => (
  <TouchableOpacity
    onPress={() => task.task !== `NA`?navigation.navigate("AppointmentDetail"):null}
    style={[styles.item, { backgroundColor: "#FEBCC8", paddingLeft: 5 }]}
  >
    {task.task !== `NA` && (
      <View
        style={[
      ...
      >
        <Text style={styles.textDefault}>
          {moment(task.dateStart).format("hh:mm a")}
        </Text>
        <Text style={styles.textDefault}>
          {moment(task.dateEnd).format("hh:mm a")}
        </Text>
      </View>
    )}
    <View
